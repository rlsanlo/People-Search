...
async function featchResults(searchField) {
  ...
  usersData = users.results.map(result => {
    ...
    return {
      ...
      nameSearch: tratarNomes(name.first.toLowerCase()) + ' ' + tratarNomes(name.last.toLowerCase()),
      gender,
      age: dob.age,
  ...
  searchField = tratarNomes(searchField.toLowerCase());
